...
class wpdb2 {
	...
	function __construct( $dbuser, $dbpassword, $dbname, $dbhost ) {
		...
		if ( function_exists( 'mysqli_connect' ) ) {
			...
			} elseif ( version_compare( phpversion(), '5.5', '>=' ) || ! function_exists( 'wpsql_connect' ) ) {
	 ...
	 * @since 3.1.0
	 *
	 * @param resource $dbh     The resource given by wpsql_connect
	...
	function set_charset( $dbh, $charset = null, $collate = null ) {
		...
		if ( $this->has_cap( 'collation' ) && ! empty( $charset ) ) {
			if ( $this->use_mysqli ) {
				...
				if ( function_exists( 'wpsql_set_charset' ) && $this->has_cap( 'set_charset' ) ) {
					wpsql_set_charset( $charset, $dbh );
				} else {
					$query = $this->prepare( 'SET NAMES %s', $charset );
					if ( ! empty( $collate ) )
						$query .= $this->prepare( ' COLLATE %s', $collate );
					wpsql_query( $query, $dbh );
				}
			}
		}
	}
	...
	function set_sql_mode( $modes = array() ) {
		if ( empty( $modes ) ) {
			if ( $this->use_mysqli ) {
				...
				$res = wpsql_query( 'SELECT @@SESSION.sql_mode', $this->dbh );
			}
		...
		if ( $this->use_mysqli ) {
			...
			wpsql_query( "SET SESSION sql_mode='$modes_str'", $this->dbh );
		}
	}
	...
	function select( $db, $dbh = null ) {
		...
		if ( $this->use_mysqli ) {
			...
			$success = @wpsql_select_db( $db, $dbh );
		}
		if ( ! $success ) {
			$this->ready = false;
...
<p>If you don\'t know how to set up a database you should <strong>contact your host</strong>. If all else fails you may find help at the <a href="https://wordpress.org/support/">WordPress Support Forums</a>.</p>' ), htmlspecialchars( $db, ENT_QUOTES ), htmlspecialchars( $this->dbuser, ENT_QUOTES ) ), 'db_select_fail' );
	...
	/**
	 * Real escape, using mysqli_real_escape_string() or wpsql_real_escape_string()
	 *
	 * @see mysqli_real_escape_string()
	 * @see wpsql_real_escape_string()
	 * @since 2.8.0
	 * @access private
	 *
	...
	function _real_escape( $string ) {
		if ( $this->dbh ) {
			if ( $this->use_mysqli ) {
				...
				return wpsql_real_escape_string( $string, $this->dbh );
			}
		}
	...
	function db_connect( $allow_bail = true ) {
		...
		if ( $this->use_mysqli ) {
			...
			// like wpsql_connect does. This duplicates how wpsql_connect detects a port and/or socket file.
			$port = null;
			$socket = null;
			$host = $this->dbhost;
			...
			if ( $this->dbh->connect_errno ) {
				...
				if ( $this->has_connected ) {
					...
					$attempt_fallback = false;
				} else if ( ! function_exists( 'wpsql_connect' ) ) {
					$attempt_fallback = false;
				}
			...
			if ( WP_DEBUG ) {
				$this->dbh = wpsql_connect( $this->dbhost, $this->dbuser, $this->dbpassword, $new_link, $client_flags );
			} else {
				$this->dbh = @wpsql_connect( $this->dbhost, $this->dbuser, $this->dbpassword, $new_link, $client_flags );
			}
		}
...
" ), htmlspecialchars( $this->dbhost, ENT_QUOTES ) ), 'db_connect_fail' );
	...
	function query( $query ) {
		...
		// if ( ! empty( $this->dbh ) ) {
		// 	if ( $this->use_mysqli ) {
		// 		$wpsql_errno = mysqli_errno( $this->dbh );
		// 	} else {
		// 		$wpsql_errno = wpsql_errno( $this->dbh );
		// 	}
		// }

		if ( empty( $this->dbh ) || 2006 == $wpsql_errno ) {
		...
		if ( $this->use_mysqli ) {
			...
			$this->last_error = wpsql_error( $this->dbh );
		}
		...
		if ( preg_match( '/^\s*(create|alter|truncate|drop)\s/i', $query ) ) {
			...
			if ( $this->use_mysqli ) {
				...
				$this->rows_affected = wpsql_affected_rows( $this->dbh );
			}
			...
			if ( preg_match( '/^\s*(insert|replace)\s/i', $query ) ) {
				if ( $this->use_mysqli ) {
					...
					$this->insert_id = wpsql_insert_id( $this->dbh );
				}
			}
			...
			if ( $this->use_mysqli ) {
				...
				while ( $row = @wpsql_fetch_object( $this->result ) ) {
					$this->last_result[$num_rows] = $row;
					$num_rows++;
				}
			}
	...
	/**
	 * Internal function to perform the wpsql_query() call.
	 *
	 * @since 3.9.0
	 *
	 * @access private
	...
	private function _do_query( $query ) {
		...
		if ( $this->use_mysqli ) {
			...
			$this->result = @wpsql_query( $query, $this->dbh );
		}
		$this->num_queries++;
	...
	protected function load_col_info() {
		...
		if ( $this->use_mysqli ) {
			...
			for ( $i = 0; $i < @wpsql_num_fields( $this->result ); $i++ ) {
				$this->col_info[ $i ] = @wpsql_fetch_field( $this->result, $i );
			}
		}
	}
	 ...
	 * @since 2.5.0
	 * @uses $wp_version
	 * @uses $required_wpsql_version
	 *
	 * @return WP_Error
	 */
	function check_database_version() {
		global $wp_version, $required_wpsql_version;
		// Make sure the server has the required MySQL version
		if ( version_compare($this->db_version(), $required_wpsql_version, '<') )
			return new WP_Error('database_version', sprintf( __( '<strong>ERROR</strong>: WordPress %1$s requires MySQL %2$s or higher' ), $wp_version, $required_wpsql_version ));
	}
	...
	function db_version() {
		if ( $this->use_mysqli ) {
			...
			$server_info = wpsql_get_server_info( $this->dbh );
		}
